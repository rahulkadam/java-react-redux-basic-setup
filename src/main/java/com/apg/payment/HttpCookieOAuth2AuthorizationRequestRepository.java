...
public class HttpCookieOAuth2AuthorizationRequestRepository implements AuthorizationRequestRepository<OAuth2AuthorizationRequest> {
    ...
    private static final String AUTHORIZATION_REQUEST_COOKIE_NAME = "ULM_oauth2_authorization_request";
    public static final String ULM_REDIRECT_URI_COOKIE_PARAM_NAME = "ULM_redirect_uri";
    ...
    public void saveAuthorizationRequest(OAuth2AuthorizationRequest authorizationRequest, HttpServletRequest request,
        ...
        String lemonRedirectUri = request.getParameter(ULM_REDIRECT_URI_COOKIE_PARAM_NAME);
        if (!StringUtils.isEmpty(lemonRedirectUri)) {

            cookie = new Cookie(ULM_REDIRECT_URI_COOKIE_PARAM_NAME, lemonRedirectUri);
            cookie.setPath("/");
            cookie.setHttpOnly(true);
            cookie.setMaxAge(cookieExpirySecs);
            response.addCookie(cookie);
        ...
        if (cookies != null && cookies.length > 0)
            for (int i = 0; i < cookies.length; i++)
                if (cookies[i].getName().equals(AUTHORIZATION_REQUEST_COOKIE_NAME) ||
                        cookies[i].getName().equals(ULM_REDIRECT_URI_COOKIE_PARAM_NAME)) {
